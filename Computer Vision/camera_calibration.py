...
# 1016, 760
class Calibration:
    def __init__(self, dirpaths=[], prefix='', image_size=None, image_format='jpg'):
        for dirpath in dirpaths:
            if not os.path.isdir(dirpath):
                raise ValueError(f'Invalid directory path: {dirpath}')

        if image_size is None:
            raise ValueError('Image size has to be a tuple')
    ...
    def undistort(self, img, alpha):
        h, w = img.shape[:2]
        newcameramtx, roi = cv.getOptimalNewCameraMatrix(
            self.camera_matrix, self.dist_coefficients, (w,h), alpha, (w,h))
        return cv.undistort(
            img, self.camera_matrix, self.dist_coefficients, None, newcameramtx)
...
class CharUcoCalibration(Calibration):
    ...
    def process_frames(self, show_results=False):
        ...
        for path in paths:
            ...
            channels, *shape = frame.shape[::-1]
            if self.image_size != tuple(shape):
                continue

            gray = cv.cvtColor(frame, cv.COLOR_BGR2GRAY)

            corners, ids, rejected_points = cv.aruco.detectMarkers(gray, self.board.dictionary)
            if len(ids) <= 0:
                continue
    ...
    def calibrate(self, camera_matrix=None, distortion_coeff=None, show_results=False):
        ...
        success, camera_matrix, distortion_coeff, rvecs, tvecs = cv.aruco.calibrateCameraCharuco(*args)
        if success:
...
class ChessboardCalibration(Calibration):
    ...
    def _get_board(self, pattern):
        match = re.match('(\d*)x(\d*)_(\d*)', pattern)
        if match:
            squares_x = int(match.group(1))
